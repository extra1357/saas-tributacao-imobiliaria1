import { comparePFvsPJ } from "../comparador";

export interface BreakevenInput {
  monthsRented?: number;
  deductions?: {
    iptu?: number; condominio?: number; administracao?: number;
    seguro?: number; juroFinanciamento?: number; despesasCobranca?: number;
  };
...
export interface BreakevenResult {
  ...
  breakevenAnnual: number | null;
  pfTaxAtBreakeven: number | null;
  pjTaxAtBreakeven: number | null;
  message: string;
}
...
export function calculateBreakeven(input: BreakevenInput = {}): BreakevenResult {
  ...
  const compare = (rev: number) => comparePFvsPJ({
